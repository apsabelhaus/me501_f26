...
%% Q5b: Simulating a Point Mass Moving in One Dimension, dynamics in a separate function
...
title('ME501 HW1 Q5b -- Point Mass 1D position vs time');
xlabel('time (sec)');
ylabel('x1 (meters)');
...
if save_video
    % save a video. Setup:
    video_filename_Q5b = "hw1_me501_Q5b";
    video_profile_name = "Motion JPEG AVI"; % this is the default for matlab
    vwriter = VideoWriter(video_filename_Q5b, video_profile_name);
    ...
    % start the plot:
    figure; hold on;
    title('ME501 HW1 Q5b -- Point Mass 1D Position Trajectory');
    xlabel('x1 (meters)');
    ...
    % watch
    close(vwriter);
    disp("Video save completed to " + video_filename_Q5b);
end
